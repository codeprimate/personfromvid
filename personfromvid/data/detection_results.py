...
class PoseDetection:
    ...
    bbox: Tuple[int, int, int, int]  # (x1, y1, x2, y2)
    confidence: float
    keypoints: Dict[str, Tuple[float, float, float]]  # {keypoint_name: (x, y, confidence)}
...
class HeadPoseResult:
    ...
    def __post_init__(self):
        ...
        if abs(self.yaw) > 180 or abs(self.pitch) > 90 or abs(self.roll) > 180:
            import warnings
            warnings.warn(f"Extreme angle values detected: yaw={self.yaw}, pitch={self.pitch}, roll={self.roll}")
...
class CloseupDetection:
    ...
    face_position: Optional[Tuple[str, str]] = None  # ("center", "upper"), rule of thirds position
...
class QualityMetrics:
    ...
    def is_high_quality(self) -> bool:
        ...
        return (
            self.overall_quality >= 0.7 and
            self.laplacian_variance >= 100.0 and
            self.usable
        )
