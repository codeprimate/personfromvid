...
class FrameExtractor:
    ...
    def extract_frames(self, output_dir: Path, 
                      progress_callback: Optional[callable] = None) -> List[FrameData]:
        ...
        try:
            ...
            self.logger.info(f"Frame extraction completed: {len(extracted_frames)} frames "
    ...
    def _extract_i_frames(self) -> List[FrameCandidate]:
        ...
        try:
            ...
            self.logger.warning(f"I-frame extraction failed: {e}, falling back to temporal sampling only")
            return []
    ...
    def _combine_and_deduplicate_candidates(self, 
                                          i_frames: List[FrameCandidate],
                                          temporal: List[FrameCandidate]) -> List[FrameCandidate]:
        ...
        for candidate in all_candidates:
            if candidate.timestamp - last_timestamp >= duplicate_threshold:
                ...
                if (deduplicated and 
                    candidate.confidence > deduplicated[-1].confidence):
                    deduplicated[-1] = candidate
                self.stats['duplicates_removed'] += 1
        
        # Apply max frames per second limit
        if len(deduplicated) > self.video_metadata.duration * self.max_frames_per_second:
            # Keep frames with highest confidence, spread across time
            target_count = int(self.video_metadata.duration * self.max_frames_per_second)
            ...
            self.logger.info(f"Limited to {target_count} frames (max {self.max_frames_per_second}/sec)")
        
        return deduplicated
    
    def _extract_frame_images(self, 
        ...
        try:
            ...
            for i, candidate in enumerate(candidates):
                try:
                    ...
                    ret, frame = cap.read()
                    if not ret:
                        self.logger.warning(f"Could not read frame at {candidate.timestamp}s")
                        continue
                    ...
                    self.logger.warning(f"Failed to extract frame at {candidate.timestamp}s: {e}")
                    continue
    ...
    def get_extraction_statistics(self) -> Dict[str, Any]:
        ...
        return {
            'total_candidates_considered': (
                self.stats['i_frames_found'] + 
                self.stats['temporal_samples']
            ),
            ...
            'coverage_percentage': (
                ...
                self.video_metadata.duration * 100
                if self.video_metadata.duration > 0 else 0
            ),
