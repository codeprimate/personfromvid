...
class FrameSelector:
    ...
    def __init__(self, criteria: Optional[SelectionCriteria] = None):
        ...
        self.head_angle_categories = [
            ...
            "looking_up_left", "looking_up_right"
        ]
    ...
    def select_best_frames(self, 
                          ...
                          progress_callback: Optional[Callable[[str], None]] = None) -> SelectionSummary:
        ...
        for category in self.pose_categories:
            ...
            available_candidates = [
                frame for frame in candidate_frames if frame.frame_id not in claimed_frame_ids
            ]
            
            if available_candidates:
                selection = self._select_for_category(
                    available_candidates, category, "pose",
                ...
                if selection.selected_frames:
                    ...
                    self.logger.debug(f"Selected and claimed {len(selection.selected_frames)} frames for pose '{category}'")
        ...
        for angle, frames in head_angle_groups.items():
            if frames:
                selection = self._select_for_category(
                    frames, angle, "head_angle",
                    self._calculate_head_angle_frame_score
                )
                head_angle_selections[angle] = selection
                self.logger.debug(f"Selected {len(selection.selected_frames)} frames for head angle '{angle}'")
        ...
        self.logger.info(f"Frame selection completed: {total_selected} unique frames selected")
        return summary
    ...
    def group_by_head_angle(self, frames: List[FrameData]) -> Dict[str, List[FrameData]]:
    ...
    def _filter_usable_frames(self, frames: List[FrameData]) -> List[FrameData]:
        ...
        for frame in frames:
            ...
            if frame.quality_metrics.overall_quality < self.criteria.min_quality_threshold:
                continue
    ...
    def _select_for_category(self, 
                           frames: List[FrameData], 
                           category_name: str,
                           category_type: str,
                           score_function: Callable[[FrameData], float]) -> CategorySelection:
        ...
        quality_scores = [f.quality_metrics.overall_quality for f in frames if f.quality_metrics]
        ...
        rationale = self._create_selection_rationale(
            category_name, category_type, len(frames), len(selected_frames),
            quality_range, average_quality
        )
    ...
    def _select_diverse_frames(self, 
                              scored_frames: List[Tuple[FrameData, float]], 
                              max_count: int) -> List[FrameData]:
    ...
    def _is_diverse_enough(self, candidate: FrameData, selected: List[FrameData]) -> bool:
    ...
    def _calculate_head_angle_frame_score(self, frame: FrameData) -> float:
        ...
        final_score = (
            ...
            (1.0 - self.criteria.quality_weight - self.criteria.face_size_weight) * head_pose_score
        )
        
        return min(1.0, max(0.0, final_score))
    
    def _create_selection_rationale(self, 
                                  category_name: str,
                                  category_type: str, 
                                  total_candidates: int,
                                  selected_count: int,
                                  quality_range: Tuple[float, float],
                                  average_quality: float) -> str:
        ...
        if selected_count > 0:
            ...
            if category_type == "head_angle":
                rationale_parts.append("Selection prioritized face size and image quality.")
            else:
                rationale_parts.append("Selection prioritized overall frame and pose quality.")
                
            if selected_count < total_candidates:
                rationale_parts.append(f"Diversity filtering applied to avoid similar frames.")
        
        return " ".join(rationale_parts)
    
    def _update_frame_selection_metadata(self, 
                                       pose_selections: Dict[str, CategorySelection],
                                       head_angle_selections: Dict[str, CategorySelection]) -> None:
