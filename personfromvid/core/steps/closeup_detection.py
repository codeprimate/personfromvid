    ...
    def execute(self) -> None:
        ...
        try:
            ...
            if not frames_with_faces:
                if self.formatter:
                    self.formatter.print_warning("No frames with faces found for closeup detection")
                else:
                    self.logger.warning("⚠️  No frames with faces found for closeup detection")
            ...
            if self.formatter:
                self.formatter.print_info("🎯 Analyzing frame composition...", 'targeting')
            else:
                self.logger.info(f"🎯 Starting closeup detection on {total_frames} frames...")
            ...
            if self.formatter:
                with self.formatter.create_progress_bar("Analyzing composition", total_frames):
                    closeup_detector.process_frame_batch(frames_with_faces, progress_callback)
            else:
                closeup_detector.process_frame_batch(frames_with_faces, progress_callback)
            ...
            for frame in frames_with_faces:
                for detection in frame.closeup_detections:
                    closeup_counts[detection.shot_type] = closeup_counts.get(detection.shot_type, 0) + 1
            ...
            self.state.get_step_progress(self.step_name).set_data("shot_types_found", closeup_counts)
            self.state.get_step_progress(self.step_name).set_data("total_closeups", total_closeups)
            
            good_composition = len([s for s in composition_scores if s >= 0.6])
            
            if self.formatter:
                sorted_shots = sorted(closeup_counts.items(), key=lambda x: x[1], reverse=True)[:4]
                ...
                results = {
                    ...
                    "composition_quality": f"✨ Good composition: {good_composition}/{len(composition_scores)} ({(good_composition/len(composition_scores)*100):.1f}%)" if composition_scores else "✨ No scores available"
                }
                self.state.get_step_progress(self.step_name).set_data("step_results", results)
            else:
                self.logger.info(f"✅ Closeup detection completed: {total_closeups} detections")
                if closeup_counts:
                    for shot_type, count in sorted(closeup_counts.items(), key=lambda x: x[1], reverse=True)[:3]:
                        self.logger.info(f"      • {shot_type}: {count} instances")
                if composition_scores:
                    self.logger.info(f"   ✨ Good composition: {good_composition}/{len(composition_scores)} frames")
