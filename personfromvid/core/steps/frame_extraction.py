...
class FrameExtractionStep(PipelineStep):
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            frame_extractor = FrameExtractor(str(self.pipeline.video_path), video_metadata)
            ...
            def progress_callback(current: int, total: int):
                ...
                if self.formatter and hasattr(self.formatter, 'update_step_progress'):
                    rate = current / (time.time() - step_start_time) if step_start_time and time.time() > step_start_time else 0
            ...
            if self.formatter and hasattr(self.formatter, 'step_progress_context'):
                with self.formatter.step_progress_context("Extracting frames", estimated_total_frames):
                    extracted_frames = frame_extractor.extract_frames(frames_dir, progress_callback)
            else:
                self.logger.info("🎬 Starting frame extraction...")
                extracted_frames = frame_extractor.extract_frames(frames_dir, progress_callback)
            
            # Update state with final results
            self.state.get_step_progress(self.step_name).total_items = len(extracted_frames)
            ...
            self.state.get_step_progress(self.step_name).set_data("extraction_stats", extraction_stats)
            ...
            if self.formatter:
                ...
                self.logger.info(f"   📊 I-frames: {extraction_stats['i_frames_found']}, "
