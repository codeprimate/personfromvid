...
REQUIRED_KEYPOINTS_STANDING = ['left_hip', 'right_hip', 'left_knee', 'right_knee', 'left_ankle', 'right_ankle']
REQUIRED_KEYPOINTS_SITTING = ['left_hip', 'right_hip', 'left_knee', 'right_knee']
REQUIRED_KEYPOINTS_SQUATTING = ['left_hip', 'right_hip', 'left_knee', 'right_knee', 'left_ankle', 'right_ankle']
...
class PoseClassifier:
    ...
    def classify_pose(self, pose_detection: PoseDetection, image_shape: Tuple[int, int]) -> List[Tuple[str, float]]:
        ...
        try:
            ...
            if self._is_closeup(keypoints, bbox, image_shape):
                classifications.append(("closeup", self._calculate_closeup_confidence(keypoints, bbox, image_shape)))
            
            if self._is_standing(keypoints):
                classifications.append(("standing", self._calculate_standing_confidence(keypoints)))
            
            if self._is_sitting(keypoints):
                classifications.append(("sitting", self._calculate_sitting_confidence(keypoints)))

            if self._is_squatting(keypoints):
                classifications.append(("squatting", self._calculate_squatting_confidence(keypoints)))
    ...
    def _is_closeup(self, keypoints: Dict[str, Tuple[float, float, float]], 
        ...
        if (left_shoulder and right_shoulder and 
            left_shoulder[2] >= self.min_keypoint_confidence and 
            right_shoulder[2] >= self.min_keypoint_confidence):
        ...
        lower_body_count = sum(1 for kp_name in lower_body_keypoints 
                              if kp_name in keypoints and keypoints[kp_name][2] >= self.min_keypoint_confidence)
    ...
    def _is_standing(self, keypoints: Dict[str, Tuple[float, float, float]]) -> bool:
        ...
        right_hip_knee_angle = self._calculate_hip_knee_angle_flexible(keypoints, 'right')
        
        # Check if legs appear extended (standing)
        left_standing = left_hip_knee_angle is not None and left_hip_knee_angle >= self.standing_hip_knee_min
        right_standing = right_hip_knee_angle is not None and right_hip_knee_angle >= self.standing_hip_knee_min
        ...
        if left_standing or right_standing:
            ...
            if torso_angle is not None and abs(torso_angle) <= self.torso_vertical_threshold:
                return True
    ...
    def _is_sitting(self, keypoints: Dict[str, Tuple[float, float, float]]) -> bool:
        ...
        left_sitting = (left_hip_knee_angle is not None and 
                       self.sitting_hip_knee_min <= left_hip_knee_angle <= self.sitting_hip_knee_max)
        right_sitting = (right_hip_knee_angle is not None and 
                        self.sitting_hip_knee_min <= right_hip_knee_angle <= self.sitting_hip_knee_max)
    ...
    def _is_squatting(self, keypoints: Dict[str, Tuple[float, float, float]]) -> bool:
        ...
        left_squatting = left_hip_knee_angle is not None and left_hip_knee_angle <= self.squatting_hip_knee_max
        right_squatting = right_hip_knee_angle is not None and right_hip_knee_angle <= self.squatting_hip_knee_max
    ...
    def _has_required_keypoints(self, keypoints: Dict[str, Tuple[float, float, float]], 
        ...
        for kp_name in required:
            if (kp_name not in keypoints or 
                keypoints[kp_name][2] < self.min_keypoint_confidence):
                return False
        return True
    
    def _calculate_hip_knee_angle(self, keypoints: Dict[str, Tuple[float, float, float]], 
    ...
    def _calculate_hip_knee_angle_flexible(self, keypoints: Dict[str, Tuple[float, float, float]], 
                                         side: str) -> Optional[float]:
        ...
        angle_from_vertical = self._calculate_angle_between_vectors(hip_knee_vec, vertical_vec)
    ...
    def _hips_above_knees(self, keypoints: Dict[str, Tuple[float, float, float]]) -> bool:
        ...
        if (left_hip and left_knee and 
            left_hip[2] >= self.min_keypoint_confidence and 
            left_knee[2] >= self.min_keypoint_confidence):
            valid_pairs.append((left_hip[1], left_knee[1]))  # y-coordinates
        
        if (right_hip and right_knee and 
            right_hip[2] >= self.min_keypoint_confidence and 
            right_knee[2] >= self.min_keypoint_confidence):
    ...
    def _calculate_torso_angle(self, keypoints: Dict[str, Tuple[float, float, float]]) -> Optional[float]:
        ...
        if not all(kp and kp[2] >= self.min_keypoint_confidence 
    ...
    def _calculate_angle_between_vectors(self, vec1: Tuple[float, float], 
    ...
    def _calculate_closeup_confidence(self, keypoints: Dict[str, Tuple[float, float, float]], 
        ...
        if (left_shoulder and right_shoulder and 
            ...
            shoulder_confidence = min(1.0, shoulder_width_ratio / self.closeup_shoulder_width_threshold)
        ...
        lower_body_keypoints = ['left_hip', 'right_hip', 'left_knee', 'right_knee', 'left_ankle', 'right_ankle']
        missing_count = sum(1 for kp_name in lower_body_keypoints 
                           if kp_name not in keypoints or keypoints[kp_name][2] < self.min_keypoint_confidence)
    ...
    def _calculate_standing_confidence(self, keypoints: Dict[str, Tuple[float, float, float]]) -> float:
        ...
        for side in ['left', 'right']:
            ...
            if angle is not None:
                if angle >= self.standing_hip_knee_min:
                    angle_confidence = min(1.0, (angle - self.standing_hip_knee_min) / 20.0 + 0.7)
    ...
    def _calculate_sitting_confidence(self, keypoints: Dict[str, Tuple[float, float, float]]) -> float:
    ...
    def _calculate_squatting_confidence(self, keypoints: Dict[str, Tuple[float, float, float]]) -> float:
        ...
        for side in ['left', 'right']:
            ...
            if angle is not None:
                if angle <= self.squatting_hip_knee_max:
                    ...
                    angle_confidence = max(0.5, 1.0 - angle / self.squatting_hip_knee_max)
                    confidence_factors.append(angle_confidence)
    ...
    def set_angle_thresholds(self, standing_min: Optional[float] = None,
                            sitting_min: Optional[float] = None,
                            sitting_max: Optional[float] = None,
                            squatting_max: Optional[float] = None) -> None:
        ...
        logger.info(f"Updated angle thresholds: standing_min={self.standing_hip_knee_min}, "
