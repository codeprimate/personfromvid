...
class ProcessingPipeline:
    ...
    def _initialize_state_management(self) -> None:
        ...
        if existing_state:
            ...
            self.logger.info(f"Loaded existing pipeline state, currently at step: {self.state.current_step}")
        else:
    ...
    def _execute_pipeline_steps(self, start_from: Optional[str] = None) -> ProcessingResult:
        """Execute all pipeline steps in sequence."""
        step_map = {step.step_name: step for step in self._steps}
        steps_to_execute = [
            (name, step_map[name]) for name in get_pipeline_step_names() if name in step_map
        ]
        ...
        if start_from:
            try:
                ...
                raise StateManagementError(f"Resume step '{start_from}' not found.")
        
        for i, (step_name, step_instance) in enumerate(steps_to_execute[start_index:], start_index + 1):
            ...
            if self.formatter:
                self.formatter.start_step(i, step_name, step_name.replace('_', ' ').title())
            else:
                self.logger.info(f"STEP {i}/{get_total_pipeline_steps()}: {step_name.replace('_', ' ').upper()}")
            ...
            try:
                ...
                if self.formatter:
                    ...
                    self.logger.info(f"✅ Step completed in {time.time() - self._step_start_time:.1f}s")
                
            except Exception as e:
                if self.formatter:
                    self.formatter.print_error(f"Step '{step_name}' failed: {e}", step_name)
                else:
    ...
    def _extract_video_metadata(self) -> VideoMetadata:
        ...
        self.logger.info(f"Video metadata extracted: {metadata.width}x{metadata.height}, {metadata.duration:.1f}s, {metadata.fps:.1f}fps")
        return metadata
    ...
    def _create_success_result(self) -> ProcessingResult:
        ...
        if head_angles_found:
            ...
            sorted_head_poses = sorted(head_angles_found.items(), key=lambda x: x[1], reverse=True)
