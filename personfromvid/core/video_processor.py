...
class VideoProcessor:
    ...
    def extract_metadata(self) -> VideoMetadata:
        ...
        try:
            ...
            self.logger.info(f"Video metadata extracted: {metadata.resolution_string}, "
            ...
            raise VideoProcessingError(f"Unexpected error during metadata extraction: {e}")
    ...
    def validate_format(self) -> bool:
        ...
        try:
            ...
            supported_codecs = {
                ...
                'mpeg4', 'xvid', 'divx', 'wmv3', 'msmpeg4v3'
            }
            ...
            if not format_supported:
                self.logger.warning(f"Potentially unsupported format: {metadata.format}")
            ...
            if metadata.width < 100 or metadata.height < 100:
                raise VideoProcessingError(f"Video resolution too low: {metadata.resolution_string}")
            
            if metadata.file_size_bytes < 1024:  # Less than 1KB
                raise VideoProcessingError("Video file appears to be corrupted (too small)")
            
            self.logger.info(f"Video format validation passed: {metadata.format}/{metadata.codec}")
            return True
            ...
            raise VideoProcessingError(f"Unexpected error during format validation: {e}")
    ...
    def _estimate_processing_time(self, metadata: VideoMetadata) -> Dict[str, float]:
        ...
        resolution_factor = (metadata.width * metadata.height) / (1920 * 1080)  # Relative to 1080p
