...
class HeadAngleClassifier:
    ...
    def _validate_angles(self, yaw: float, pitch: float, roll: float) -> None:
        ...
        if not all(isinstance(angle, (int, float)) and not (angle != angle) for angle in [yaw, pitch, roll]):
            raise HeadAngleClassificationError(f"Invalid angle values: yaw={yaw}, pitch={pitch}, roll={roll}")
        
        # Warn about extreme values but don't fail
        if abs(yaw) > 180 or abs(pitch) > 90 or abs(roll) > 180:
            warnings.warn(f"Extreme angle values detected: yaw={yaw}, pitch={pitch}, roll={roll}")
    
    def _calculate_direction_confidence(self, direction: str, yaw: float, pitch: float, 
                                      roll: float, base_confidence: float) -> float:
        ...
        if abs(roll) > self.max_roll:
            roll_penalty = 1.0 - (abs(roll) - self.max_roll) / 60.0  # Reduce confidence for high roll
    ...
    def _calculate_angle_centeredness(self, direction: str, yaw: float, pitch: float) -> float:
    ...
    def set_angle_thresholds(self, yaw: Optional[float] = None, pitch: Optional[float] = None,
        ...
        logger.info(f"Updated angle thresholds: yaw={self.yaw_threshold}°, "
