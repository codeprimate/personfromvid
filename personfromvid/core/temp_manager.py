...
class TempManager:
    ...
    def create_temp_structure(self) -> Path:
        ...
        try:            
            ...
            self.logger.info(f"Created temporary directory structure: {self.temp_dir_path}")
    ...
    def cleanup_temp_files(self) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(f"Temp directory size: {dir_size / (1024*1024):.1f} MB")
            except Exception:
                dir_size = 0
            ...
            if self.temp_dir_path.exists():
                ...
                if dir_size > 0:
                    self.logger.info(f"Freed {dir_size / (1024*1024):.1f} MB of disk space")
    ...
    def get_temp_file_path(self, filename: str, subdir: Optional[str] = None) -> Path:
        ...
        if subdir:
            if subdir == "frames":
                ...
                raise ValueError(f"Unknown subdirectory: {subdir}. Only 'frames' is supported.")
        else:
