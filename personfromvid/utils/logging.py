...
class PersonFromVidLogger:
    ...
    def _setup_logging(self) -> None:
        ...
        if self.config.enable_rich_console:
            install_rich_traceback(console=self.console, show_locals=self.config.verbose)
        ...
        if self.config.enable_rich_console and self.console:
            ...
            console_handler.setFormatter(
                PersonFromVidFormatter(
                    ...
                    include_module=self.config.verbose
                )
            )
    ...
    def get_logger(self, name: str) -> logging.Logger:
        ...
        if name not in self._loggers:
            logger_name = f"personfromvid.{name}" if not name.startswith("personfromvid") else name
    ...
    def log_exception(self, exception: Exception, logger_name: str = "main") -> None:
        ...
        if isinstance(exception, PersonFromVidError):
            ...
            logger.error(format_exception_message(exception, include_traceback=self.config.verbose))
        else:
