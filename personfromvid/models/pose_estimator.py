...
class PoseEstimator:
    ...
    def __init__(self, model_name: str, device: str = DEFAULT_DEVICE, confidence_threshold: float = DEFAULT_CONFIDENCE_THRESHOLD):
        ...
        if not self.model_config.is_device_supported(device_type):
            raise PoseEstimationError(f"Model {model_name} does not support device {device}")
    ...
    def _load_model(self) -> None:
        ...
        try:
            if self._model_format == ModelFormat.PYTORCH:
                ...
                raise PoseEstimationError(f"Unsupported model format: {self._model_format}")
                
            logger.debug(f"Successfully loaded {self.model_name} model")
            
        except Exception as e:
            raise PoseEstimationError(f"Failed to load model {self.model_name}: {str(e)}") from e
    ...
    def _load_pytorch_model(self) -> None:
        ...
        try:
            ...
            raise PoseEstimationError("Required dependencies not installed. For YOLO: pip install ultralytics") from e
    ...
    def _load_onnx_model(self) -> None:
        ...
        try:
            ...
            raise PoseEstimationError("onnxruntime not installed. Install with: pip install onnxruntime") from e
    ...
    def estimate_pose(self, image: np.ndarray) -> List[PoseDetection]:
        ...
        try:
            ...
            classified_detections = self.classify_pose_detections(detections, image_shape)
    ...
    def _estimate_pytorch(self, image: np.ndarray) -> List[PoseDetection]:
        ...
        if "yolo" in self.model_name.lower():
            ...
            raise PoseEstimationError(f"Generic PyTorch models not yet implemented for {self.model_name}")
    ...
    def _estimate_batch_pytorch(self, images: List[np.ndarray]) -> List[List[PoseDetection]]:
    ...
    def _estimate_batch_onnx(self, images: List[np.ndarray]) -> List[List[PoseDetection]]:
    ...
    def _postprocess_yolo_results(self, result, image_shape: Tuple[int, int]) -> List[PoseDetection]:
        ...
        if hasattr(result, 'boxes') and result.boxes is not None and hasattr(result, 'keypoints') and result.keypoints is not None:
            ...
            if (hasattr(boxes, 'xyxy') and hasattr(boxes, 'conf') and 
                hasattr(keypoints, 'xy') and hasattr(keypoints, 'conf') and
                keypoints.xy is not None and keypoints.conf is not None):
    ...
    def _postprocess_onnx_results(self, outputs: List[np.ndarray], image_shape: Tuple[int, int]) -> List[PoseDetection]:
    ...
    def validate_pose_detection(self, detection: PoseDetection, image_shape: Tuple[int, int]) -> bool:
    ...
    def calculate_pose_confidence(self, detection: PoseDetection) -> float:
        ...
        weighted_sum = sum(conf * weight for conf, weight in zip(top_confidences, weights))
        weight_sum = sum(weights)
        
        return weighted_sum / weight_sum if weight_sum > 0 else 0.0
    
    def classify_pose_detections(self, detections: List[PoseDetection], image_shape: Tuple[int, int]) -> List[PoseDetection]:
    ...
    def process_frame_batch(self, frames_with_faces: List['FrameData'], 
        ...
        logger.info(f"Starting body pose estimation on {total_frames} frames ({total_batches} batches)")
        ...
        for i in range(0, total_frames, batch_size):
            ...
            logger.debug(f"Processing pose estimation batch {batch_num}/{total_batches} ({len(batch_frames)} frames)")
            ...
            try:
                ...
                for j, (frame_data, poses) in enumerate(zip(batch_frame_data, batch_pose_results)):
                    if poses:
                        ...
                        for pose in poses:
                            if pose.pose_classifications:
                                for classification, _ in pose.pose_classifications:
                                    poses_by_category[classification] = poses_by_category.get(classification, 0) + 1
                                    batch_classifications[classification] = batch_classifications.get(classification, 0) + 1
                
                # Log batch results
                if batch_poses_found > 0:
                    logger.debug(f"Batch {batch_num}/{total_batches}: {batch_poses_found} poses found, classifications: {batch_classifications}")
                else:
                    logger.debug(f"Batch {batch_num}/{total_batches}: No poses detected")
                
            except Exception as e:
                logger.error(f"Pose estimation failed for batch {batch_num}/{total_batches}: {e}")
        ...
        logger.debug(f"Body pose estimation completed: {total_poses_found} poses found in {total_frames} frames")
        logger.debug(f"Final pose categories: {poses_by_category}")
        
        return poses_by_category, total_poses_found


def create_pose_estimator(model_name: Optional[str] = None, device: str = "auto", 
                         confidence_threshold: float = DEFAULT_CONFIDENCE_THRESHOLD) -> PoseEstimator:
