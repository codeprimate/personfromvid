...
class ProgressManager:
    ...
    def update_pipeline_state(self, pipeline_state: PipelineState) -> None:
        ...
        if self.main_progress:
            ...
            if pipeline_state.current_step in pipeline_state.step_progress:
                current_step_progress = pipeline_state.step_progress[pipeline_state.current_step].progress_percentage / 100.0
        ...
        self._update_step_progress(pipeline_state.current_step, pipeline_state.step_progress)
        ...
        if self.live_display and hasattr(self.live_display, 'renderable'):
            ...
            layout["main"].update(Panel(self.main_progress, title="Overall Progress", border_style="blue"))
            layout["step"].update(Panel(self.step_progress, title="Current Step", border_style="green"))
            layout["stats"].update(Panel(self.stats_table or "", title="Processing Statistics", border_style="yellow"))
    ...
    def update_step_progress(self, step_name: str, processed_count: int, 
    ...
    def add_statistics_panel(self, stats: Dict[str, Any]) -> None:
        ...
        if self.pipeline_state:
            self.stats_table.add_row("Video File", str(self.pipeline_state.video_file.split('/')[-1]))
            self.stats_table.add_row("Current Step", self.pipeline_state.current_step.replace('_', ' ').title())
            self.stats_table.add_row("Completed Steps", f"{len(self.pipeline_state.completed_steps)}/{get_total_pipeline_steps()}")
            
            # Add step-specific stats
            current_step_progress = self.pipeline_state.step_progress.get(self.pipeline_state.current_step)
            if current_step_progress:
                self.stats_table.add_row("Step Progress", f"{current_step_progress.progress_percentage:.1f}%")
                
                if self.pipeline_state.current_step in self.step_stats:
                    step_stat = self.step_stats[self.pipeline_state.current_step]
                    self.stats_table.add_row("Processing Rate", f"{step_stat.items_per_second:.1f} items/s")
                    self.stats_table.add_row("Peak Rate", f"{step_stat.peak_rate:.1f} items/s")
    ...
    def display_final_summary(self, result: ProcessingResult) -> None:
        ...
        summary_table = Table(title="Processing Complete", show_header=True, header_style="bold blue")
        ...
        summary_table.add_row("Processing Time", f"{result.processing_time_seconds:.1f}s")
        ...
        # Display summary
        self.console.print()
        self.console.print(Panel(summary_table, border_style="green" if result.success else "red"))
        ...
        if result.output_files:
            ...
            if len(result.output_files) > 10:
                self.console.print(f"  ... and {len(result.output_files) - 10} more files")
    ...
    def _update_step_progress(self, current_step: str, step_progress_dict: Dict[str, StepProgress]) -> None:
    ...
    def _update_statistics_panel(self) -> None:
        ...
        if self.pipeline_state and self.pipeline_state.current_step in self.pipeline_state.step_progress:
            current_progress = self.pipeline_state.step_progress[self.pipeline_state.current_step]
            
            # Add counts from step data
            if "extracted_count" in current_progress.step_data:
                stats["Frames Extracted"] = str(current_progress.get_data("extracted_count", 0))
