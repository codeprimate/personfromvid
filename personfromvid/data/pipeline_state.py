...
class PipelineState:
    ...
    def is_step_completed(self, step_name: str) -> bool:
        ...
        return (
            step_name in self.completed_steps and
            step_name in self.step_progress and
            self.step_progress[step_name].completed
        )
    ...
    def get_poses_found(self) -> Dict[str, int]:
        ...
        for frame in self.frames:
            for pose_classification in frame.get_pose_classifications():
                poses_by_category[pose_classification] = poses_by_category.get(pose_classification, 0) + 1
    ...
    def get_head_angles_found(self) -> Dict[str, int]:
        ...
        for frame in self.frames:
            for head_direction in frame.get_head_directions():
                head_angles_by_category[head_direction] = head_angles_by_category.get(head_direction, 0) + 1
    ...
    def get_frame_selections(self) -> Dict[str, Any]:
        ...
        if "frame_selection" in self.step_progress:
            return self.step_progress["frame_selection"].get_data("frame_selections", {})
        return {}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PipelineState":
        ...
        for step_name, progress_data in data.get("step_progress", {}).items():
            ...
            if progress_data.get("started_at"):
                progress.started_at = datetime.fromisoformat(progress_data["started_at"])
        ...
        for frame_dict in data.get("frames", []):
            try:
                ...
                import logging
                logging.getLogger("pipeline_state").warning(f"Failed to deserialize frame: {e}")
...
class PipelineStatus:
    ...
    completed_steps: List[str] = field(default_factory=list)
    total_steps: int = field(default_factory=get_total_pipeline_steps)  # Total number of processing steps
