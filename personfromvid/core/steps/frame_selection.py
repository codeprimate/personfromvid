...
class FrameSelectionStep(PipelineStep):
    ...
    def execute(self) -> None:
        ...
        try:
            candidate_frames = [
                f for f in self.state.frames 
                if f.has_faces() and f.has_poses() and f.quality_metrics is not None
            ]

            if not candidate_frames:
                if self.formatter:
                    self.formatter.print_warning("No frames with quality assessments for selection")
                else:
                    self.logger.warning("⚠️ No frames with quality assessments for selection")
                self.state.get_step_progress(self.step_name).start(0)
                return

            total_candidates = len(candidate_frames)
            if self.formatter:
                self.formatter.print_info("🎯 Optimizing frame selection...", 'targeting')
            else:
            ...
            def progress_callback(message: str):
                ...
                if self.formatter: self.formatter.update_progress(1)

            if self.formatter:
                with self.formatter.create_progress_bar("Selecting frames", total_candidates):
                    selection_summary = frame_selector.select_best_frames(candidate_frames, progress_callback)
            else:
                selection_summary = frame_selector.select_best_frames(candidate_frames, lambda m: self.logger.debug(f"   {m}"))

            self.state.update_step_progress(self.step_name, total_candidates) # Mark as complete
    ...
    def _store_selection_results(self, selection_summary, criteria):
        ...
        for category, selection in selection_summary.pose_selections.items():
            if selection.selected_frames:
                frame_selections["pose_categories"][category] = {
                    ...
                    "quality_stats": {"range": selection.quality_range, "average": selection.average_quality},
                }
        ...
        self.state.get_step_progress(self.step_name).set_data("frame_selections", frame_selections)
        ...
        self.state.get_step_progress(self.step_name).set_data(ALL_SELECTED_FRAMES_KEY, unique_frame_ids)

    def _format_and_log_results(self, selection_summary):
        """Formats the results for display and logs them."""
        if self.formatter:
            body_poses = [f"{cat} ({len(sel.selected_frames)})" for cat, sel in selection_summary.pose_selections.items() if sel.selected_frames]
            head_angles = [f"{cat} ({len(sel.selected_frames)})" for cat, sel in selection_summary.head_angle_selections.items() if sel.selected_frames]
            ...
            results = {
                ...
                "body_poses_breakdown": f"🏃 Body poses: {', '.join(body_poses)}" if body_poses else "🏃 No body poses selected",
                "head_angles_breakdown": f"👤 Head angles: {', '.join(head_angles)}" if head_angles else "👤 No head angles selected"
            }
            self.state.get_step_progress(self.step_name).set_data("step_results", results)
        else:
            self.logger.info(f"✅ Frame selection completed: {selection_summary.total_selected} frames from {selection_summary.total_candidates} candidates")
            if selection_summary.total_selected == 0:
                self.logger.warning("⚠️  No frames were selected - check quality thresholds!") 