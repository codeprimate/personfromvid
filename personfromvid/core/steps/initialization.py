...
class InitializationStep(PipelineStep):
    ...
    def execute(self) -> None:
        ...
        try:
            if self.formatter:
                self.formatter.print_info("🔧 Setting up processing environment...", 'setup')
            else:
            ...
            if self.formatter:
                progress_bar = self.formatter.create_progress_bar("Setting up environment")
                with progress_bar:
                    self.pipeline.video_processor.validate_format()
                    video_info = self.pipeline.video_processor.get_video_info_summary()
                    self.state.get_step_progress(self.step_name).set_data("video_info", video_info)
            else:
            ...
            if self.formatter:
                ...
                self.logger.info(f"✅ Video validated: {video_info['resolution']}, "
