    ...
    def execute(self) -> None:
        ...
        try:
            ...
            if not frames_with_faces:
                if self.formatter:
                    self.formatter.print_warning("No frames with faces found, skipping pose analysis")
                else:
                    self.logger.info("⚠️  No frames with faces found, skipping pose analysis")
            ...
            if self.formatter:
                with self.formatter.create_progress_bar("Analyzing body poses", total_frames):
                    poses_by_category, _ = pose_estimator.process_frame_batch(frames_with_faces, body_progress_callback)
                with self.formatter.create_progress_bar("Analyzing head angles", total_frames):
                    head_angles_by_category, _ = head_pose_estimator.process_frame_batch(frames_with_faces, head_progress_callback)
            else:
                poses_by_category, _ = pose_estimator.process_frame_batch(frames_with_faces, body_progress_callback)
                head_angles_by_category, _ = head_pose_estimator.process_frame_batch(frames_with_faces, head_progress_callback)

            # Store results
            total_poses_found = sum(len(frame.pose_detections) for frame in frames_with_faces)
            total_head_poses_found = sum(len(frame.head_poses) for frame in frames_with_faces)
            
            self.state.get_step_progress(self.step_name).set_data("poses_found", poses_by_category)
            self.state.get_step_progress(self.step_name).set_data("head_angles_found", head_angles_by_category)
            self.state.get_step_progress(self.step_name).set_data("total_poses_found", total_poses_found)
            self.state.get_step_progress(self.step_name).set_data("total_head_poses_found", total_head_poses_found)
            self.state.get_step_progress(self.step_name).set_data("total_analyzed", len(frames_with_faces))
            
            top_poses = sorted(poses_by_category.items(), key=lambda x: x[1], reverse=True)[:3]
            top_head_poses = sorted(head_angles_by_category.items(), key=lambda x: x[1], reverse=True)[:3]
            ...
            if self.formatter:
                ...
                self.logger.info(f"   📊 Body poses: {total_poses_found} across {len(poses_by_category)} categories")
                self.logger.info(f"   📊 Head poses: {total_head_poses_found} across {len(head_angles_by_category)} categories")
                if top_poses:
                    self.logger.info(f"   🏆 Top body poses: {', '.join([f'{name} ({count})' for name, count in top_poses])}")
                if top_head_poses:
                    self.logger.info(f"   🏆 Top head poses: {', '.join([f'{name} ({count})' for name, count in top_head_poses])}")
