...
class SelectionInfo:
    ...
    selected_for_poses: List[str] = field(default_factory=list)  # List of pose categories this frame was selected for
    selected_for_head_angles: List[str] = field(default_factory=list)  # List of head angle categories
    ...
    crop_regions: Dict[str, Tuple[int, int, int, int]] = field(default_factory=dict)  # Named crop regions
...
class FrameData:
    ...
    def complete_processing_step(self, step_name: str, processing_time_ms: Optional[float] = None) -> None:
    ...
    def is_high_quality(self) -> bool:
        ...
        return (
            self.quality_metrics is not None and
            self.quality_metrics.is_high_quality
        )
    ...
    def from_dict(cls, frame_dict: Dict[str, Any]) -> 'FrameData':
        ...
        """
        from .detection_results import (
            FaceDetection, PoseDetection, HeadPoseResult, 
            QualityMetrics, CloseupDetection, ProcessingTimings
        )
        ...
        for pose_dict in frame_dict.get('pose_detections', []):
            pose_detection = PoseDetection(
                ...
                pose_classifications=[tuple(pc) for pc in pose_dict.get('pose_classifications', [])]
            )
        ...
        for closeup_dict in frame_dict.get('closeup_detections', []):
            closeup_detection = CloseupDetection(
                ...
                face_position=tuple(closeup_dict.get('face_position', [])) if closeup_dict.get('face_position') else None
            )
