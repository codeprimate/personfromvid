    ...
    def execute(self) -> None:
        ...
        try:
            ...
            face_detector = create_face_detector(
                ...
                device=self.config.models.device.value if hasattr(self.config.models.device, 'value') else str(self.config.models.device),
            ...
            if self.formatter:
                progress_bar = self.formatter.create_progress_bar("Processing frames", total_frames)
                with progress_bar:
            ...
            coverage = (frames_with_faces / total_frames * 100) if total_frames > 0 else 0
            
            self.state.get_step_progress(self.step_name).set_data("faces_found", total_faces_found)
            ...
            if self.formatter:
                ...
                self.logger.info(f"✅ Face detection completed: {total_faces_found} faces found")
                self.logger.info(f"   📊 Frames with faces: {frames_with_faces}/{total_frames}")
