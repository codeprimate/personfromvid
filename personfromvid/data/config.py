...
class ModelConfig(BaseModel):
    ...
    head_pose_model: str = Field(
        default="sixdrepnet",
        description="Head pose estimation model name"
    )
    device: DeviceType = Field(
        default=DeviceType.AUTO,
        description="Computation device preference"
    )
...
class FrameExtractionConfig(BaseModel):
    ...
    enable_keyframe_detection: bool = Field(
        default=True,
        description="Enable I-frame keyframe detection"
    )
    enable_temporal_sampling: bool = Field(
        default=True,
        description="Enable temporal frame sampling"
    )
    ...
    deduplication_enabled: bool = Field(
        default=True,
        description="Enable frame deduplication"
    )
...
class QualityConfig(BaseModel):
    ...
    brightness_min: float = Field(
        ...
        ge=0.0,
        le=255.0,
        description="Minimum acceptable brightness"
    )
    brightness_max: float = Field(
        default=225.0,
        ge=0.0,
        le=255.0,
        description="Maximum acceptable brightness"
    )
    ...
    enable_multiple_metrics: bool = Field(
        default=True,
        description="Use multiple quality metrics"
    )
...
class HeadAngleConfig(BaseModel):
    ...
    max_roll_degrees: float = Field(
        ...
        ge=15.0,
        le=60.0,
        description="Maximum acceptable roll angle"
    )
...
class CloseupDetectionConfig(BaseModel):
    ...
    enable_composition_analysis: bool = Field(
        default=True,
        description="Enable frame composition assessment"
    )
...
class JpegConfig(BaseModel):
    ...
    quality: int = Field(95, ge=70, le=100, description="Quality for JPEG images (1-100).")
...
class OutputConfig(BaseModel):
    ...
    preserve_metadata: bool = Field(
        default=True,
        description="Preserve metadata in output images"
    )
...
class ProcessingConfig(BaseModel):
    ...
    enable_resume: bool = Field(
        default=True,
        description="Enable processing resumption from checkpoints"
    )
...
class LoggingConfig(BaseModel):
    ...
    log_file: Optional[Path] = Field(
        default=None,
        description="Log file path (None for auto-generated)"
    )
