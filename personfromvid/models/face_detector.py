...
class FaceDetector:
    ...
    def __init__(self, model_name: str, device: str = DEFAULT_DEVICE, confidence_threshold: float = DEFAULT_CONFIDENCE_THRESHOLD):
        ...
        if not self.model_config.is_device_supported(device_type):
            raise FaceDetectionError(f"Model {model_name} does not support device {device}")
    ...
    def _load_model(self) -> None:
        ...
        try:
            if self._model_format == ModelFormat.ONNX:
                ...
                raise FaceDetectionError(f"Unsupported model format: {self._model_format}")
                
            logger.debug(f"Successfully loaded {self.model_name} model")
            
        except Exception as e:
            raise FaceDetectionError(f"Failed to load model {self.model_name}: {str(e)}") from e
    ...
    def _load_onnx_model(self) -> None:
        ...
        try:
            ...
            raise FaceDetectionError("onnxruntime not installed. Install with: pip install onnxruntime") from e
    ...
    def _load_pytorch_model(self) -> None:
        ...
        try:
            ...
            raise FaceDetectionError("Required dependencies not installed. For YOLO: pip install ultralytics") from e
    ...
    def _detect_pytorch(self, image: np.ndarray) -> List[FaceDetection]:
        ...
        if "yolo" in self.model_name.lower():
            ...
            raise FaceDetectionError(f"Generic PyTorch models not yet implemented for {self.model_name}")
    ...
    def _detect_batch_pytorch(self, images: List[np.ndarray]) -> List[List[FaceDetection]]:
    ...
    def _postprocess_onnx_results(self, outputs: List[np.ndarray], image_shape: Tuple[int, int]) -> List[FaceDetection]:
        ...
        if len(outputs) == 9:
            ...
            for scale_idx in range(3):
                ...
                if np.any(valid_indices):
                    ...
                    anchors = self._generate_anchors(scale_idx, input_height, input_width)
        ...
        # Fallback for simpler model formats
            ...
            landmarks = outputs[2] if len(outputs) > 2 else None  # Shape: (N, 10) for 5 points
            ...
            if np.any(valid_indices):
                valid_bboxes = bboxes[valid_indices]
                valid_scores = scores[valid_indices]
                valid_landmarks = landmarks[valid_indices] if landmarks is not None else None
    ...
    def _generate_anchors(self, scale_idx: int, input_height: int, input_width: int) -> np.ndarray:
    ...
    def _postprocess_yolo_results(self, result, image_shape: Tuple[int, int]) -> List[FaceDetection]:
    ...
    def validate_face_detection(self, detection: FaceDetection, image_shape: Tuple[int, int]) -> bool:
    ...
    def process_frame_batch(self, frames: List['FrameData'], 
        ...
        for i in range(0, len(frames), batch_size):
            ...
            try:
                ...
                for j, (faces, frame) in enumerate(zip(batch_face_results, valid_frames)):
                ...
                logger.error(f"Face detection failed for batch {i//batch_size + 1}: {e}")
        ...
        logger.info(f"Face detection completed: {total_faces_found} faces found in "
                   f"{processed_count} frames")

    def _create_frame_data_from_info(self, frame_info: Dict, faces: List['FaceDetection'], 
                                   image: Optional[np.ndarray], 
                                   video_metadata: 'VideoMetadata') -> 'FrameData':
...
def create_face_detector(model_name: Optional[str] = None, device: str = "auto", 
                        confidence_threshold: float = DEFAULT_CONFIDENCE_THRESHOLD) -> FaceDetector:
