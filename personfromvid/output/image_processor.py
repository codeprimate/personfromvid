...
from .crop_utils import calculate_fixed_aspect_ratio_bbox, parse_aspect_ratio
...
class ImageProcessor:
    ...
    def __init__(self, config: OutputImageConfig):
        ...
        self.logger.debug(
            f"ImageProcessor initialized with crop_ratio: {config.crop_ratio}"
        )
        self.logger.debug(
            f"ImageProcessor initialized with enable_pose_cropping: {config.enable_pose_cropping}"
        )
    ...
    def expand_bbox_to_aspect_ratio(
        self, bbox: Tuple[int, int, int, int], image: np.ndarray, padding: float = 0.0
        ...
        if self.config.crop_ratio is not None:
            ...
            if self.config.crop_ratio == "any":
                self.logger.debug(
                    "crop_ratio='any': using variable aspect ratio with padding"
                )
                return padded_bbox
    ...
    def crop_and_resize(
        ...
        if extend_left > 0 or extend_top > 0 or extend_right > 0 or extend_bottom > 0:
            ...
            if image.shape[2] == 3:  # RGB
                ...
                alpha_channel = np.full(
                    (image_height, image_width, 1), 255, dtype=image.dtype
                )
            ...
            expanded_image = np.zeros(
                (expanded_height, expanded_width, 4), dtype=image.dtype
            )

            # Copy original image into expanded canvas
            expanded_image[
                extend_top : extend_top + image_height,
                extend_left : extend_left + image_width,
            ] = image_rgba
        ...
        min_dimension = (
            ...
            else self.config.default_crop_size
        )
        ...
        if use_face_restoration and needs_upscaling:
            ...
            if face_restorer is not None:
                try:
                    ...
                    target_size = min(
                        min_dimension, self.config.default_crop_size
                    )  # Cap at default_crop_size for performance
                    ...
                    self.logger.warning(
                        f"Face restoration failed, falling back to Lanczos: {e}"
                    )
