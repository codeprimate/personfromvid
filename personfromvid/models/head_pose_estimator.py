...
class HeadPoseEstimator:
    ...
    def __init__(self, model_name: str, device: str = DEFAULT_DEVICE, confidence_threshold: float = DEFAULT_CONFIDENCE_THRESHOLD):
        ...
        if not self.model_config:
            raise HeadPoseEstimationError(f"Unknown head pose estimation model: {model_name}")
        ...
        if not self.model_config.is_device_supported(device_type):
            raise HeadPoseEstimationError(f"Model {model_name} does not support device {device}")
        ...
        logger.info(f"Initialized HeadPoseEstimator with model {model_name} on {device}")
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            elif self._model_format == ModelFormat.PYTORCH or self._model_format == ModelFormat.SAFETENSORS:
                self._load_pytorch_model()
            else:
                raise HeadPoseEstimationError(f"Unsupported model format: {self._model_format}")
                
            logger.debug(f"Successfully loaded {self.model_name} model")
            
        except Exception as e:
            raise HeadPoseEstimationError(f"Failed to load model {self.model_name}: {str(e)}") from e
    ...
    def _load_hopenet_model(self) -> None:
        ...
        try:
            ...
            raise HeadPoseEstimationError("PyTorch not installed. Install with: pip install torch torchvision") from e
    ...
    def _load_onnx_model(self) -> None:
        ...
        try:
            ...
            raise HeadPoseEstimationError("onnxruntime not installed. Install with: pip install onnxruntime") from e
    ...
    def _load_pytorch_model(self) -> None:
        ...
        if self.model_path.suffix == '.safetensors':
            try:
                ...
                raise HeadPoseEstimationError("safetensors not installed. Install with: pip install safetensors")
        else:
    ...
    def _create_sixdrepnet_model(self):
        ...
        class SixDRepNet(nn.Module):
            def __init__(self):
                ...
                self.layer0 = RepVGGBlock(3, 48, kernel_size=3, stride=2, padding=1)  # RepVGG-A0 width
                
                # Layer 1: 48 -> 48 (2 blocks)
                self.layer1 = nn.Sequential(
                    RepVGGBlock(48, 48, stride=2),
                    RepVGGBlock(48, 48, stride=1)
                )
    ...
    def estimate_head_pose(self, face_image: np.ndarray) -> HeadPoseResult:
        ...
        try:
            ...
            raise HeadPoseEstimationError(f"Head pose estimation failed: {str(e)}") from e
    ...
    def estimate_batch(self, face_images: List[np.ndarray]) -> List[HeadPoseResult]:
        ...
        for i, image in enumerate(face_images):
            if image is None or image.size == 0:
                raise HeadPoseEstimationError(f"Input face image at index {i} is empty or None")
        ...
        try:
            ...
            raise HeadPoseEstimationError(f"Batch head pose estimation failed: {str(e)}") from e
    ...
    def _estimate_batch_hopenet(self, face_images: List[np.ndarray]) -> List[HeadPoseResult]:
        ...
        for i in range(0, len(face_images), batch_size):
            ...
            with torch.no_grad():
                ...
                for j in range(len(batch_images)):
                    ...
                    # Convert to degrees and normalize
                    yaw_degrees = self._normalize_angle(float(yaw_predicted.cpu().numpy()))
    ...
    def _estimate_batch_onnx(self, face_images: List[np.ndarray]) -> List[HeadPoseResult]:
    ...
    def _estimate_batch_pytorch(self, face_images: List[np.ndarray]) -> List[HeadPoseResult]:
    ...
    def _postprocess_onnx_results(self, outputs: List[np.ndarray]) -> HeadPoseResult:
        ...
        if len(outputs) >= 3:
            yaw = float(outputs[0].item()) if outputs[0].size == 1 else float(outputs[0][0])
            pitch = float(outputs[1].item()) if outputs[1].size == 1 else float(outputs[1][0])
            roll = float(outputs[2].item()) if outputs[2].size == 1 else float(outputs[2][0])
            ...
            raise HeadPoseEstimationError(f"Unexpected ONNX output format: {[o.shape for o in outputs]}")
    ...
    def _softmax_to_angle(self, predictions):
        ...
        # Create angle bins
        idx_tensor = torch.arange(predictions.size(1), dtype=torch.float32, device=predictions.device)
    ...
    def set_forward_facing_thresholds(self, yaw: Optional[float] = None, pitch: Optional[float] = None) -> None:
        ...
        logger.debug(f"Updated forward-facing thresholds: yaw={self.forward_yaw_threshold}, pitch={self.forward_pitch_threshold}")
    
    def set_angle_thresholds(self, yaw: Optional[float] = None, pitch: Optional[float] = None, 
                           profile_yaw: Optional[float] = None, max_roll: Optional[float] = None) -> None:
        ...
        logger.debug(f"Updated angle thresholds: yaw={self.yaw_threshold}, pitch={self.pitch_threshold}, "
    ...
    def process_frame_batch(self, frames_with_faces: List['FrameData'], 
        ...
        batch_size = 8  # Process fewer head pose images per batch due to memory constraints
        total_frames = len(frames_with_faces)
        total_batches = (total_frames + batch_size - 1) // batch_size
        
        logger.info(f"Starting head pose estimation on {total_frames} frames ({total_batches} batches)")
        
        for i in range(0, total_frames, batch_size):
            batch_num = i // batch_size + 1
            batch_frames = frames_with_faces[i:i + batch_size]
            
            logger.debug(f"Processing head pose batch {batch_num}/{total_batches} ({len(batch_frames)} frames)")
            ...
            for frame_data in batch_frames:
                try:
                    ...
                    logger.warning(f"Failed to process frame {frame_data.file_path}: {e}")
                    continue
            
            # Run head pose estimation on face crops
            if batch_face_images:
                try:
                    logger.debug(f"Running head pose inference on {len(batch_face_images)} face crops...")
                    ...
                    for j, (head_pose, frame_data, face_idx) in enumerate(zip(batch_head_poses, batch_frame_data, batch_face_indices)):
                        ...
                        head_angles_by_category[direction] = head_angles_by_category.get(direction, 0) + 1
                        batch_directions[direction] = batch_directions.get(direction, 0) + 1
                    
                    # Log batch results
                    if batch_head_poses_found > 0:
                        logger.debug(f"Batch {batch_num}/{total_batches}: {batch_head_poses_found} head poses found, "
        ...
        logger.debug(f"Head pose estimation completed: {total_head_poses_found} head poses found in {total_frames} frames")
...
def create_head_pose_estimator(model_name: Optional[str] = None, device: str = "auto", 
                              confidence_threshold: float = DEFAULT_CONFIDENCE_THRESHOLD) -> HeadPoseEstimator:
