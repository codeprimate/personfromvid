...
class ConsolidatedFormatter:
    ...
    def start_processing(self, video_path: str, config_info: Dict[str, Any]) -> None:
        ...
        if 'video_metadata' in config_info and config_info['video_metadata']:
            ...
            self.console.print(f"   📹 {width}×{height}, {fps:.1f}fps, {duration_str} ({size_mb:.1f}MB)")
    ...
    def start_step(self, step_number: int, step_name: str, description: str) -> None:
        ...
        if self.enable_debug:
            self.console.print(f"[dim]DEBUG: Starting step {step_number}: {step_name}[/dim]")
    ...
    def step_progress_context(self, description: str, total: Optional[int] = None):
        ...
        self.current_step_task = self.step_progress.add_task(
            description, 
            total=total,
            rate="0/s" if total else "working..."
        )

        self.live_display = Live(self.step_progress, console=self.console, transient=True)
    ...
    def update_step_progress(self, advance: int = 1, **kwargs) -> None:
        ...
        if self.current_step_task is not None and self.step_progress is not None:
            ...
            self.step_progress.update(
                self.current_step_task, 
                advance=advance,
                **update_data
            )
    
    def complete_step(self, step_name: str, results: Optional[Dict[str, Any]] = None) -> None:
        ...
        duration = self.step_timings[step_name].duration_formatted if step_name in self.step_timings else "unknown"
    ...
    def _print_step_details(self, step_name: str, results: Optional[Dict[str, Any]] = None) -> None:
        ...
        if step_name == "initialization":
            ...
            if 'extraction_summary' in results:
                ...
                match = re.search(r'Extracted (\d+) unique frames', results['extraction_summary'])
                if match:
                    frame_count = match.group(1)
                    self.console.print(f"  • Extracted {frame_count} frames from video")
            elif 'total_frames' in results:
                self.console.print(f"  • Extracted {results['total_frames']} frames from video")
            ...
            if 'extraction_summary' in results:
                match = re.search(r'\((\d+) duplicates removed\)', results['extraction_summary'])
                if match:
            ...
            if 'faces_found' in results and 'frames_with_faces' in results:
                self.console.print(f"  • Found {results['faces_found']} faces in {results['frames_with_faces']} frames")
            
        elif step_name == "pose_analysis":
            if 'poses_found' in results and 'head_angles_found' in results:
                self.console.print(f"  • Analyzed poses for {results.get('total_analyzed', 0)} detected faces")
                ...
                if head_angles:
                    head_items = [f"{name} ({count})" for name, count in head_angles.items()]
                    self.console.print(f"  • Head poses found: {', '.join(head_items)}")
                    
        elif step_name == "closeup_detection":
            if 'total_closeups' in results:
                self.console.print(f"  • Identified {results['total_closeups']} frames as containing a closeup")
            ...
            if 'output_files' in results:
                self.console.print(f"  • Writing {len(results['output_files'])} images to disk:")
                for file_info in results['output_files']:
                    if isinstance(file_info, dict) and 'filename' in file_info and 'frame_number' in file_info:
                        self.console.print(f"    - {file_info['filename']} (from frame {file_info['frame_number']})")
    ...
    def print_completion_summary(self, result: ProcessingResult, output_path: str) -> None:
        ...
        if result.success:
            ...
            self.console.print(f"📊 Results: {result.total_frames_extracted} frames, {result.faces_found} faces, {len(result.output_files)} files")
            ...
            if result.output_files:
                output_dir = Path(result.output_files[0]).parent if result.output_files else Path(output_path).parent
...
def create_consolidated_formatter(console: Optional[Console] = None, enable_debug: bool = False) -> ConsolidatedFormatter:
